...
- **2026-04-08 (audit):** Fixed fitbit_sync.py sleep score field. Was using `efficiency` (% time in bed asleep) instead of Fitbit's actual Sleep Score. Now uses `main.get("score", efficiency)` — prefers real score, falls back to efficiency for older devices. **Why:** User saw 93 in sheet but 76 in Fitbit app. 93 was efficiency, 76 was the actual Sleep Score. Different metrics. **Connections:** fitbit_sync.py → Recovery tab (Sleep Score column).
...
- **2026-04-09 (audit-2):** Hardened verify and blank-row detection. (1) `_verify_sheet_write` now retries: tries immediately, waits 2s and retries once on failure — no wasted delay on the happy path. (2) `_find_next_row` now detects gaps (blank rows between data rows) and logs a warning — monitoring, not intervention, so writes still succeed. (3) Fixed blank row in Cardio tab (row 4 between Apr 7 and Apr 9). (4) Updated memory.md — Seated Row Machine now replaces Cable Rows (was incorrectly reversed). (5) Added blank-row check to audit template Section 5. **Why:** All 5 workout verifies failed despite correct writes — likely Google Sheets API propagation delay. Fixed delay (1.5s) was wasteful; retry is zero-cost when verify passes first try. Blank rows are the recurring root cause of column drift (Apr 7, Apr 9) — gap detection surfaces them in logs before they cause damage. **Connections:** bot.py (`_verify_sheet_write`, `_find_next_row`), Cardio tab, memory.md, daily-audit-template.md.
